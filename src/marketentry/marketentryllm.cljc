(ns marketentry.marketentryllm
  ...
  the Mongolia (MNG) public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real legal-entity registration /
  investment filing. Every output is censored downstream by
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world legal-entity registration /
  investment filing request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け法人設立登記/投資申請提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "investor-foreign-state-ownership-pct=" (:investor-foreign-state-ownership-pct e)
                        " proposed-ownership-pct=" (:proposed-ownership-pct e)
                        " sector=" (:sector e)
                        " med-permission-verified?=" (:med-permission-verified? e)
                        " entity-registration-verified?=" (:entity-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-entity-registration? e))
                              (:entity-registration-verified? e)))
                   0.9 0.3)}))
