...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MNG, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up foreign-soe-permission-unverified) ==")
    ...
    (println "== filing/submit eng-4 (foreign-soe-permission-unverified -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up entity-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (entity-registration-unverified -> HARD hold) ==")
