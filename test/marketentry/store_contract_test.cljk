...
(defn- exercise [s]
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "MNG" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:investor-foreign-state-ownership-pct (:engagement m)) (:investor-foreign-state-ownership-pct (:engagement d))))
    (is (= (:sector (:engagement m)) (:sector (:engagement d))))
    (is (= (:med-permission-verified? (:engagement m)) (:med-permission-verified? (:engagement d))))
