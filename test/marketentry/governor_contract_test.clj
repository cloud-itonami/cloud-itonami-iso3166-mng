  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Ulaanbaatar Trading LLC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Ulaanbaatar Trading LLC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-soe-permission-unverified-is-held-and-unoverridable
  (testing "an independently recomputed MED permission requirement with no verification on file -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-soe-permission-unverified} (-> (store/ledger db) last :basis)))
...
(deftest entity-registration-unverified-is-held-and-unoverridable
  (testing "unverified legal-entity registration certificate when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:entity-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
